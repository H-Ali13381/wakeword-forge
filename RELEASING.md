...
Current package release version: `0.1.1`.
...
## Create the v0.1.1 source release

After the intended release commit is on `main`:

```bash
git tag -a v0.1.1 -m "wakeword-forge v0.1.1"
git push origin main
git push origin v0.1.1
gh release create v0.1.1 \
  --title "wakeword-forge v0.1.1" \
  --notes-file CHANGELOG.md \
  --verify-tag
```

If the release should stay unpublished while you review notes, add `--draft` to the `gh release create v0.1.1` command.
