...
from .augmentation import build_training_augmentors
from .project import background_negative_target, negative_coverage_errors, partial_negative_target
...
def run_training(config: ForgeConfig) -> Path:
    ...
    coverage_errors = negative_coverage_errors(
        background_negatives=len(neg_files),
        partial_negatives=len(partial_files),
        background_target=background_negative_target(config),
        partial_target=partial_negative_target(config),
    )
    if coverage_errors:
        raise ValueError(
            "Insufficient negative coverage: "
            + "; ".join(coverage_errors)
            + ". Generate/import more negatives before training, or use the dashboard capture step."
        )
