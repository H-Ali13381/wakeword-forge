...
def _apply_training_augmentation_options(
    config,
    *,
    augmentation: bool | None = None,
    augmentation_preset: str | None = None,
    regular_negative_preset: str | None = None,
    spectrogram_augmentation: bool | None = None,
    augmentation_noise_dir: Path | None = None,
    augmentation_ir_dir: Path | None = None,
    augmentation_short_noise_dir: Path | None = None,
    augmentation_truck_noise_dir: Path | None = None,
) -> None:
    if augmentation is not None:
        config.training_augmentation_enabled = bool(augmentation)
    if augmentation_preset is not None:
        config.training_augmentation_preset = augmentation_preset
    if regular_negative_preset is not None:
        config.regular_negative_augmentation_preset = regular_negative_preset
    if spectrogram_augmentation is not None:
        config.use_spectrogram_augmentation = bool(spectrogram_augmentation)
    if augmentation_noise_dir is not None:
        config.augmentation_noise_dir = str(augmentation_noise_dir.expanduser())
    if augmentation_ir_dir is not None:
        config.augmentation_ir_dir = str(augmentation_ir_dir.expanduser())
    if augmentation_short_noise_dir is not None:
        config.augmentation_short_noise_dir = str(augmentation_short_noise_dir.expanduser())
    if augmentation_truck_noise_dir is not None:
        config.augmentation_truck_noise_dir = str(augmentation_truck_noise_dir.expanduser())
...
def run(
    ...
    if config.tts_engine != "none" and n_confusables < CONFUSABLE_NEGATIVE_TARGET:
        ...
        synthesize_confusable_negatives(
            ...
            n_variants=CONFUSABLE_NEGATIVE_TARGET - n_confusables,
            engine=config.tts_engine,
        )

    # ── Step 4c: Background negatives ────────────────────────────────────────
    n_neg_total = count_wavs(config.negatives_path)
    background_target = background_negative_target(config)
    if n_neg_total < background_target:
        console.print(
            f"\n[bold]Step 3c:[/bold] Background negatives  "
            f"[dim]({n_neg_total} recorded/generated — target {background_target})[/dim]"
        )
...
def import_negatives(
    ...
    source_dir: Optional[Path] = typer.Option(
        None,
        "--source-dir",
        help="Folder of external negative audio to normalize/chunk/import.",
    ),
    manifest: Optional[Path] = typer.Option(
        None,
        "--manifest",
        help="JSONL negative manifest with path/source_dataset/category/license metadata.",
    ),
    kind: str = typer.Option(
        "background",
        "--kind",
        help="Target pool: background, partial, or confusable.",
    ),
    limit: Optional[int] = typer.Option(None, "--limit", help="Maximum chunks to import."),
    limit_per_source: int = typer.Option(
        100,
        "--limit-per-source",
        help="Maximum chunks imported from one source_dataset/source folder.",
    ),
    max_chunks_per_file: int = typer.Option(
        20,
        "--max-chunks-per-file",
        help="Maximum sequential chunks imported from one source audio file.",
    ),
    chunk_duration: float = typer.Option(3.0, "--chunk-duration", help="Seconds per imported chunk."),
    strata: Optional[str] = typer.Option(
        None,
        "--strata",
        help="Per-stratum quotas, e.g. speech=50,noise=50,silence=20 or a JSON object.",
    ),
    stratify_by: str = typer.Option(
        "category",
        "--stratify-by",
        help="Manifest field used for --strata quotas; defaults to category.",
    ),
) -> None:
    """Import manifest/directory negative audio with caps, chunking, and provenance."""

    from .negative_ingestion import import_negative_audio, parse_strata_quotas

    config = load_or_create_config(project_dir)
    ensure_project_dirs(config)
    try:
        strata_quotas = parse_strata_quotas(strata)
    ...
    save_config(config)
    console.print(
        f"[green]Imported {result.imported_count} negative chunk(s)[/green] "
        f"from {result.available_count} source record(s) into {result.target_dir}."
    )
    if result.skipped_paths:
        console.print(f"[yellow]Skipped {len(result.skipped_paths)} source file(s)/record(s).[/yellow]")
    if result.strata_counts:
        summary = ", ".join(f"{name}={count}" for name, count in sorted(result.strata_counts.items()))
        console.print(f"[dim]Strata imported: {summary}[/dim]")
    console.print(f"[dim]Provenance manifest: {result.manifest_path}[/dim]")
...
def train(
    ...
    augmentation: Optional[bool] = typer.Option(
        ...
        help="Enable or disable training-time acoustic augmentation for this project.",
    ),
    ...
    regular_negative_preset: Optional[str] = typer.Option(
        None,
        "--regular-negative-preset",
        help="Waveform augmentation preset for background negatives: light, standard, or none.",
    ),
    spectrogram_augmentation: Optional[bool] = typer.Option(
        None,
        "--spectrogram-augmentation/--no-spectrogram-augmentation",
        help="Enable or disable SpecAugment-style mel masking during training.",
    ),
    augmentation_noise_dir: Optional[Path] = typer.Option(
        None,
        "--augmentation-noise-dir",
        help="Folder of local background-noise WAVs mixed into augmented samples.",
    ),
    augmentation_ir_dir: Optional[Path] = typer.Option(
        None,
        "--augmentation-ir-dir",
        help="Folder of room impulse-response WAVs convolved into augmented samples.",
    ),
    augmentation_short_noise_dir: Optional[Path] = typer.Option(
        None,
        "--augmentation-short-noise-dir",
        help="Folder of short transient-noise WAVs mixed into augmented samples.",
    ),
    augmentation_truck_noise_dir: Optional[Path] = typer.Option(
        None,
        "--augmentation-truck-noise-dir",
        help="Folder of low-frequency rumble/noise WAVs mixed into augmented samples.",
    ),
) -> None:
    """Train using existing samples in the project directory."""
    config = load_or_create_config(project_dir)
    config.backend = backend
    _apply_training_augmentation_options(
        config,
        augmentation=augmentation,
        augmentation_preset=augmentation_preset,
        regular_negative_preset=regular_negative_preset,
        spectrogram_augmentation=spectrogram_augmentation,
        augmentation_noise_dir=augmentation_noise_dir,
        augmentation_ir_dir=augmentation_ir_dir,
        augmentation_short_noise_dir=augmentation_short_noise_dir,
        augmentation_truck_noise_dir=augmentation_truck_noise_dir,
    )
    save_config(config)
    status = inspect_project(config)
...
def info(
    ...
    t.add_row("Background negs", f"{status.negatives}/{status.background_negative_target}")
    t.add_row("Partial negs",    f"{status.partial_negatives}/{status.partial_negative_target}")
    t.add_row("Confusable negs", str(status.confusable_negatives))
