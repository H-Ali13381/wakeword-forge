...
def test_transcript_matching_accepts_contains_and_fuzzy_wakephrase_matches():
    exact = transcript_matches_phrase("The generated clip says: OK, Nova!", ["OK Nova"])
    fuzzy = transcript_matches_phrase("The generated clip says ok nva now", ["OK Nova"])
    miss = transcript_matches_phrase("The generated clip says hello there", ["OK Nova"])
    ...
    masked_short_token_miss = transcript_matches_phrase("the oak nova clip", ["OK Nova"])
    masked_token_miss = transcript_matches_phrase("the fake word detector", ["wake word"])
    split_token_miss = transcript_matches_phrase("ok no va", ["OK Nova"])
...
def test_validate_cloned_audio_rejects_silence_and_requires_wakephrase_transcript(tmp_path):
    ...
    good_result = validate_cloned_audio(good, ["Hey Nova"], transcript="hey nova")
    silent_result = validate_cloned_audio(silent, ["Hey Nova"], transcript="hey nova")
    mismatch_result = validate_cloned_audio(good, ["Hey Nova"], transcript="hello world")
...
def test_staged_cloned_sample_requires_human_decision_before_entering_training_pool(tmp_path):
    config = ForgeConfig(wake_phrase="OK Nova", project_dir=str(tmp_path))
    ...
    validation = validate_cloned_audio(generated, ["OK Nova"], transcript="OK Nova")
...
def test_staged_cloned_sample_negative_and_unusable_decisions(tmp_path):
    ...
    generated = tmp_path / "generated.wav"
    _write_wav(generated, np.ones(SAMPLE_RATE // 4, dtype=np.float32) * 0.05)
    validation = validate_cloned_audio(generated, ["OK Nova"], transcript="hello world")
...
def test_generate_one_voice_clone_sample_stops_after_first_suitable_source(tmp_path):
    config = ForgeConfig(wake_phrase="OK Nova", project_dir=str(tmp_path / "project"))
    ensure_project_dirs(config)
...
def test_qwentts_docker_job_is_one_sample_at_a_time_and_uses_project_mounts(tmp_path):
    ...
    assert job["text"] == "OK Nova"
    assert job["model_name"] == "Qwen/Qwen3-TTS-12Hz-1.7B-Base"
    assert DEFAULT_QWENTTS_MODEL == job["model_name"]
